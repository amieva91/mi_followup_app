...
@portfolio_bp.route('/asset-registry/sync-to-assets', methods=['POST'])
@login_required
def sync_assets_from_registry():
    ...
    try:
        ...
        if not assets:
            flash('❌ No hay assets con ISIN en la base de datos', 'error')
            return redirect(url_for('portfolio.asset_registry'))
        ...
        for asset in assets:
            ...
            # Verificar si hay cambios y aplicarlos
            has_changes = False
        ...
        if updated_count > 0:
            db.session.commit()
            flash(f'✅ Sincronización completada: {updated_count} assets actualizados desde AssetRegistry', 'success')
            flash(f'💡 Ahora puedes actualizar los precios con el botón "🔄 Actualizar Precios"', 'info')
        else:
            flash('ℹ️ No hay cambios para sincronizar. Todos los assets están actualizados.', 'info')
        
        if not_found_count > 0:
            flash(f'⚠️ {not_found_count} assets no encontrados en AssetRegistry', 'warning')
