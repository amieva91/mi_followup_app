...
- **REGLA DE VISUALIZACIÓN ACCIONABLE (obligatoria con ``visualization: auto``):** debes entregar los resultados visuales **exclusivamente** como **imágenes rasterizadas** (deltas de imagen raster / salida binaria embebida en la respuesta del agente). **Prohibido** incluir especificaciones JSON, esquemas Vega-Lite, JSON de layout, código o texto que solo **describa** un gráfico sin ser la imagen final. Si **no** puedes generar la imagen final en formato raster (binario/base64 por la herramienta nativa), **no incluyas** ninguna sección visual ni placeholder textual para ese gráfico: omítelo y resume solo en texto lo imprescindible.
