...
if [ "$RESTART" -eq 1 ]; then
  gcloud compute ssh "$INSTANCE" --zone="$ZONE" \
    --command="sudo -u followup git -C ${APP_DIR} pull origin main && sudo -u followup bash -lc 'cd ${APP_DIR} && FLASK_APP=run.py FLASK_ENV=production ./venv/bin/flask db upgrade' && sudo -u followup bash -lc 'cd ${APP_DIR} && ./scripts/install_all_crons.sh' && sudo ${APP_DIR}/scripts/install_logrotate_followup.sh && sudo systemctl restart followup && sleep 1 && sudo systemctl is-active followup && echo 'Listo: código actualizado, migraciones, crons, logrotate y followup reiniciado.'"
else
  gcloud compute ssh "$INSTANCE" --zone="$ZONE" \
